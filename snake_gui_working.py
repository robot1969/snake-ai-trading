# Trading Snake - Working GUI Version
# Fixed tkinter implementation

import tkinter as tk
from tkinter import Canvas, Label, Frame, messagebox
import random

class WorkingGUISnake:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("量化交易贪吃蛇 - 图形版")
        self.root.configure(bg='#2c3e50')
        self.root.resizable(False, False)
        
        # Game settings
        self.cell_size = 20
        self.grid_width = 16
        self.grid_height = 16
        self.canvas_width = self.grid_width * self.cell_size
        self.canvas_height = self.grid_height * self.cell_size
        
        # Game state
        self.snake = [(8, 8), (7, 8), (6, 8)]
        self.direction = (1, 0)
        self.money = 100000
        self.food = []
        self.trade_count = 0
        self.win_count = 0
        self.loss_count = 0
        self.teleport_count = 0
        self.game_running = False
        self.game_speed = 200
        
        # Colors
        self.colors = {
            'bg': '#2c3e50',
            'grid': '#3a5a3a3',
            'snake': '#4b8b2b',
            'snake_head': '#5a9a9a',
            'food_profit': '#4caf50',
            'food_loss': '#e74c3c',
            'panel': '#0d47a1'
        }
        
        self.setup_ui()
        self.make_food()
        self.draw_game()
        self.update_display()
    
    def setup_ui(self):
        # Main container
        main_frame = Frame(self.root, bg=self.colors['bg'])
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # Game canvas
        game_frame = Frame(main_frame, bg=self.colors['grid'])
        game_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        self.canvas = Canvas(
            game_frame,
            width=self.canvas_width,
            height=self.canvas_height,
            bg=self.colors['grid'],
            highlightthickness=0
        )
        self.canvas.pack()
        
        # Info panel
        info_frame = Frame(main_frame, bg=self.colors['panel'], width=300)
        info_frame.pack(side=tk.RIGHT, fill=tk.BOTH, padx=10, pady=10)
        info_frame.pack_propagate(False)
        
        # Title
        Label(info_frame, text="量化交易贪吃蛇", font=('Arial', 16, 'bold'),
              bg=self.colors['panel'], fg='white').pack(pady=10)
        
        # Money
        self.money_label = Label(info_frame, text="💰 资本: $100,000",
                               font=('Arial', 12),
                               bg=self.colors['panel'], fg='white').pack(pady=5)
        
        # P&L
        self.pnl_label = Label(info_frame, text="📈 盈亏: +$0",
                               font=('Arial', 12),
                               bg=self.colors['panel'], fg='white').pack(pady=5)
        
        # Stats
        self.trades_label = Label(info_frame, text="🤝 交易: 0",
                               font=('Arial', 12),
                               bg=self.colors['panel'], fg='white').pack(pady=5)
        
        self.winrate_label = Label(info_frame, text="🎯 胜率: 0.0%",
                                font=('Arial', 12),
                                bg=self.colors['panel'], fg='white').pack(pady=5)
        
        self.teleport_label = Label(info_frame, text="⚡ 传送: 0",
                               font=('Arial', 12),
                               bg=self.colors['panel'], fg='white').pack(pady=5)
        
        # Separator
        Frame(info_frame, height=2, bg='white').pack(fill=tk.X, pady=10)
        
        # Controls
        self.start_button = tk.Button(info_frame, text="🎮 开始", font=('Arial', 12, 'bold'),
                                     bg='#4caf50', fg='white',
                                     command=self.start_game,
                                     width=12, height=2)
        self.start_button.pack(pady=5)
        
        self.pause_button = tk.Button(info_frame, text="⏸️ 暂停", font=('Arial', 12),
                                     bg='#e74c3c', fg='white',
                                     command=self.pause_game,
                                     width=12, height=2, state='disabled')
        self.pause_button.pack(pady=5)
        
        # Status
        self.status_label = Label(info_frame, text="按'开始游戏'",
                               font=('Arial', 11),
                               bg=self.colors['panel'], fg='#81c784')
        self.status_label.pack(pady=10)
        
        # Instructions
        instructions_frame = Frame(info_frame, bg=self.colors['panel'])
        instructions_frame.pack(pady=10)
        
        instructions = [
            "操作说明:",
            "方向键 ↑↓←→ 或 WASD 控制",
            "",
            "💰 绿色: 盈利 +$1,500",
            "🔴 红色: 亏损 -$1,200",
            "",
            "⚡ 撞墙自动传送",
            "传送会损失金钱",
            "",
            "ESC 退出游戏"
        ]
        
        for instruction in instructions:
            Label(instructions_frame, text=instruction,
                  font=('Arial', 10),
                  bg=self.colors['panel'], fg='white').pack(anchor='w', pady=2)
        
        # Legend
        legend_frame = Frame(info_frame, bg=self.colors['panel'])
        legend_frame.pack(pady=10)
        
        Label(legend_frame, text="交易类型:", font=('Arial', 12, 'bold'),
                  bg=self.colors['panel'], fg='white').pack(anchor='w', pady=5)
        
        legend_items = [
            ("💰", "盈利交易 +$1,500"),
            ("🔴", "亏损交易 -$1,200"),
            ("⚡", "突破 +$2,000"),
            ("🔮", "反转 +$2,500")
        ]
        
        for symbol, desc in legend_items:
            item_frame = Frame(legend_frame, bg=self.colors['panel'])
            item_frame.pack(anchor='w', pady=2)
            
            Label(item_frame, text="  " + symbol, font=('Arial', 10),
                  bg=self.colors['panel'], fg='white').pack(side='left')
            Label(item_frame, text=desc, font=('Arial', 10),
                  bg=self.colors['panel'], fg='white').pack(side='left', padx=10)
        
        # Status message
        self.status_label = Label(info_frame, text="最近交易: -",
                               font=('Arial', 10),
                               bg=self.colors['panel'], fg='white').pack(anchor='w', pady=5)
        
        # Bind keyboard events
        self.root.bind('<Up>', lambda e: self.change_direction(0, -1))
        self.root.bind('<Down>', lambda e: self.change_direction(0, 1))
        self.root.bind('<Left>', lambda e: self.change_direction(-1, 0))
        self.root.bind('<Right>', lambda e: self.change_direction(1, 0))
        self.root.bind('w', lambda e: self.change_direction(0, -1))
        self.root.bind('s', lambda e: self.change_direction(0, 1))
        self.root.bind('a', lambda e: self.change_direction(-1, 0))
        self.root.bind('d', lambda e: self.change_direction(1, 0))
        self.root.bind('<space>', lambda e: self.toggle_pause())
        self.root.bind('<Escape>', lambda e: self.quit_game())
        
        self.root.focus_set()
    
    def change_direction(self, dx, dy):
        if not self.game_running:
            return
        
        current_dx, current_dy = self.direction
        if (dx == -current_dx and dy == 0) or (dx == 0 and dy == -current_dy):
            return
        if dx == current_dx and dy == current_dy:
            return
        
        self.direction = (dx, dy)
    
    def make_food(self):
        if len(self.food) < 3:
            for _ in range(2):
                x = random.randint(0, self.grid_width - 1)
                y = random.randint(0, self.grid_height - 1)
                
                if (x, y) not in self.snake and not any((x, y) == (f[0], f[1]) for f in self.food):
                    food_type = random.choice(['profit', 'profit', 'loss', 'breakout', 'reversal'])
                    self.food.append((x, y, food_type))
                    break
    
    def move_snake(self):
        if not self.game_running:
            return
        
        head_x, head_y = self.snake[0]
        new_x = head_x + self.direction[0]
        new_y = head_y + self.direction[1]
        
        # Wall collision - teleport
        if new_x < 0 or new_x >= self.grid_width or new_y < 0 or new_y >= self.grid_height:
            self.teleport_snake()
            return
        
        # Self collision - teleport
        if (new_x, new_y) in self.snake[1:]:
            self.teleport_snake()
            return
        
        self.snake.insert(0, (new_x, new_y))
        
        # Check food collision
        for i, food in enumerate(self.food):
            if (new_x, new_y) == (food[0], food[1]):
                self.eat_food(food[2], i)
                break
        
        if len(self.snake) == len(self.food):
            self.snake.pop()
        
        self.make_food()
    
    def eat_food(self, food_type, index):
        values = {
            'profit': 1500,
            'loss': -1200,
            'breakout': 2000,
            'reversal': 2500
        }
        base_pnl = values[food_type]
        pnl = int(base_pnl * random.uniform(0.8, 1.3))
        
        self.money += pnl
        self.trade_count += 1
        
        if pnl > 0:
            self.win_count += 1
            self.status_label.config(text=f"✨ 盈利! +${pnl:,}", fg='#4caf50')
            if len(self.snake) < 15:
                self.snake.append(self.snake[-1])
        else:
            self.loss_count += 1
            self.status_label.config(text=f"💸 亏损! {pnl:,}", fg='#e74c3c')
        
        self.food.pop(index)
    
    def teleport_snake(self):
        old_pos = self.snake[0]
        
        for _ in range(10):  # Limit teleport attempts
            x = random.randint(0, self.grid_width - 1)
            y = random.randint(0, self.grid_height - 1)
            
            if (x, y) not in self.snake and not any((x, y) == (f[0], f[1]) for f in self.food):
                self.snake[0] = (x, y)
                self.teleport_count += 1
                penalty = 1500 + (self.teleport_count - 1) * 300
                self.money = max(10000, self.money - penalty)
                
                self.status_label.config(text=f"⚡ 传送! 损失${penalty:,}", fg='#ff9800')
                
                if len(self.snake) > 3:
                    removed = min(2, len(self.snake) - 1)
                    for _ in range(removed):
                        if len(self.snake) > 2:
                            self.snake.pop()
                
                break
    
    def draw_game(self):
        self.canvas.delete("all")
        
        # Draw grid
        for i in range(self.grid_width + 1):
            x = i * self.cell_size
            self.canvas.create_line(x, 0, x, self.canvas_height, fill=self.colors['grid'])
        for i in range(self.grid_height + 1):
            y = i * self.cell_size
            self.canvas.create_line(0, y, self.canvas_width, y, fill=self.colors['grid'])
        
        # Draw food
        for food in self.food:
            x, y = food[0], food[1]
            food_type = food[2]
            
            # Food color
            if food_type == 'profit':
                color = self.colors['food_profit']
            elif food_type == 'loss':
                color = self.colors['food_loss']
            elif food_type == 'breakout':
                color = self.colors['food_breakout']
            elif food_type == 'reversal':
                color = self.colors['food_reversal']
            else:
                color = self.colors['food_profit']
            
            # Draw food circle
            self.canvas.create_oval(
                x + 4, y + 4, x + self.cell_size - 4, y + self.cell_size - 4,
                fill=color, outline='white', width=2
            )
        
            # Symbol
            symbols = {'profit': "$", "loss": "L", "breakout": "B", "reversal": "R"}
            symbol = symbols.get(food_type, "$")
            self.canvas.create_text(
                x + self.cell_size//2, y + self.cell_size//2,
                symbol, font=('Arial', 10, 'bold'), fill='white'
            )
        
        # Draw snake
        for i, (x, y) in enumerate(self.snake):
            if i == 0:
                # Snake head
                self.canvas.create_rectangle(
                    x + 2, y + 2, x + self.cell_size - 2, y + self.cell_size - 2,
                    fill=self.colors['snake_head'], outline='white', width=2
                )
                
                # Eyes
                eye_size = 3
                self.canvas.create_oval(
                    x + 6, y + 6, x + eye_size, y + eye_size,
                    fill='white'
                )
                self.canvas.create_oval(
                    x + self.cell_size - 7, y + 6, x + eye_size, y + eye_size,
                    fill='white'
                )
            else:
                # Snake body
                self.canvas.create_rectangle(
                    x + 3, y + 3, x + self.cell_size - 3, y + self.cell_size - 3,
                    fill=self.colors['snake'], outline='white'
                    width=1
                )
    
    def update_display(self):
        current_pnl = self.money - 100000
        win_rate = (self.win_count / max(1, self.trade_count)) * 100
        
        self.money_label.config(text=f"💰 资本: ${self.money:,}")
        
        pnl_color = '#4caf50' if current_pnl >= 0 else '#e74c3c'
        self.pnl_label.config(text=f"📈 盈亏: {current_pnl:+,}")
        self.pnl_label.config(fg=pnl_color)
        
        self.trades_label.config(text=f"🤝 交易: {self.trade_count}")
        
        win_rate_color = '#4caf50' if win_rate >= 60 else ('#ff9800' if win_rate >= 40 else '#e74c3c')
        self.winrate_label.config(text=f"🎯 胜率: {win_rate:.1f}%")
        self.winrate_label.config(fg=win_rate_color)
        
        self.teleport_label.config(text=f"⚡ 传送: {self.teleport_count}")
        
        if self.trades:
            recent_trades = self.trades[-3:]
            recent_text = "最近: " + " ".join([f"{'+ if t > 0 else ''}{t:,}" for t in recent_trades])
            self.status_label.config(text=recent_text, fg='#81c784')
    
    def start_game(self):
        if not self.game_running:
            self.game_running = True
            self.start_button.config(state='disabled', text="游戏中...")
            self.pause_button.config(state='normal')
            self.status_label.config(text="游戏进行中...", fg='#81c784')
            self.game_loop()
    
    def pause_game(self):
        if self.game_running:
            self.game_running = False
            self.pause_button.config(text="▶️ 继续")
            self.status_label.config(text="游戏暂停", fg='#ff9800')
        else:
            self.game_running = True
            self.pause_button.config(text="⏸️ 暂停")
            self.status_label.config(text="游戏进行中...", fg='#81c784')
            self.game_loop()
    
    def quit_game(self):
        if messagebox.askyesno("退出游戏", "确定要退出游戏吗？"):
            self.root.destroy()
    
    def toggle_pause(self):
        if self.game_running:
            self.pause_game()
        else:
            self.start_game()
    
    def game_loop(self):
        if self.game_running:
            self.move_snake()
            self.draw_game()
            self.update_display()
            
            # Check game over
            if self.money < 10000:
                self.game_over()
                return
            
            # Check victory
            if self.trade_count >= 30:
                self.victory()
                return
            
            # Schedule next frame
            self.root.after(self.game_speed, self.game_loop)
    
    def game_over(self):
        self.game_running = False
        self.start_button.config(state='normal', text="🎮 重新开始")
        self.pause_button.config(state='disabled')
        self.status_label.config(text="💔 游戏结束！", fg='#e74c3c')
        
        messagebox.showinfo(
            "游戏结束",
            f"资本耗尽！\n\n最终统计:\n"
            f"最终资本: ${self.money:,}\n"
            f"总交易次数: {self.trade_count}\n"
            f"胜率: {(self.win_count/max(1,self.trade_count))*100:.1f}%\n"
            f"传送次数: {self.teleport_count}\n\n"
            f"点击'重新开始'再玩一次！"
        )
        
        self.reset_game()
    
    def victory(self):
        self.game_running = False
        self.start_button.config(state='normal', text="🎮 重新开始")
        self.pause_button.config(state='disabled')
        self.status_label.config(text="🎉 胜利！", fg='#4caf50')
        
        messagebox.showinfo(
            "胜利！",
            f"恭喜完成30笔交易！\n\n最终统计:\n"
            f"最终资本: ${self.money:,}\n"
            f"净盈亏: ${self.money - 100000:+,}\n"
            f"总交易次数: {self.trade_count}\n"
            f"胜率: {(self.win_count/max(1,self.trade_count))*100:.1f}%\n"
            f"传送次数: {self.teleport_count}\n\n"
            f"你是交易大师！"
        )
        
        self.reset_game()
    
    def reset_game(self):
        self.snake = [(8, 8), (7, 8), (6, 8)]
        self.direction = (1, 0)
        self.money = 100000
        self.food = []
        self.trade_count = 0
        self.win_count = 0
        self.loss_count = 0
        self.teleport_count = 0
        self.draw_game()
        self.update_display()
        self.status_label.config(text="按'开始游戏'")
    
    def run(self):
        self.root.mainloop()

if __name__ == "__main__":
    game = WorkingGUISnake()
    game.run()