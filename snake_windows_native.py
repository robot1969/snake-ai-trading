# Trading Snake - Windows Native Graphics Version
...
class WindowsNativeSnake:
    def __init__(self):
        self.size = 20  # Larger for better experience
    ...
    def setup_console(self):
        ...
        kernel32.SetConsoleCursorInfo(kernel32.GetStdHandle(-11), ctypes.byref(CONSOLE_CURSOR_INFO(1, CONSOLE_CURSOR_INFO().bVisible))
    ...
    def draw_enhanced_interface(self):
        ...
        if current_pnl > 0:
            ...
            pnl_color = self.colors['YELLOW']
            pnl_symbol = "→"
        
        print(f" Capital: ${self.money:>10,}", end="")
        self.set_color(pnl_color)
        print(f" P&L: {current_pnl:+9,} {pnl_symbol}")
        
        self.set_color(self.colors['WHITE'])
        print(f" | Trades: {self.trade_count:3d}")
        ...
        print(f" | Teleports: {self.teleport_count:2d}")
        self.set_color(self.colors['WHITE'])
        
        # Mini chart with colors
        ...
        for y in range(self.size):
            ...
            for x in range(self.size):
                if (x,y) == self.snake[0]:
                    # Animated snake head with color
    ...
    def make_food(self):
        """Generate food with enhanced types"""
        if len(self.food) < 5:
            while True:
    ...
    def move_snake(self):
        """Move snake with enhanced mechanics"""
        head = self.snake[0]
        new = (head[0] + self.dir[0], head[1] + self.dir[1])
        
        # Wall collision - enhanced teleport
    ...
    def take_trade_enhanced(self, trade_type):
        """Enhanced trade handling with visual feedback"""
        values = {
            "profit": 1500, 
            "loss": -1200, 
            "breakout": 2000, 
        ...
        if pnl > 0:
            ...
            if len(self.snake) < 15:
                ...
                print(f"  Snake length: {len(self.snake)} segments")
        else:
            self.loss_count += 1
            self.loss_total += pnl
    ...
    def teleport_enhanced(self):
        ...
        while True:
            ...
            if (x,y) not in self.snake and not any((x,y)==(f[0],f[1]) for f in self.food):
                ...
                self.set_color(self.colors['RED'])
                print(f"  Teleport Cost: ${penalty:,}")
                
                # Sound effect simulation with colors
                for _ in range(3):
                    self.set_color(self.colors['MAGENTA'])
                    print("  ✨✨✨ TELEPORTING! ✨✨✨")
                    time.sleep(0.05)
                ...
                if len(self.snake) > 3:
                    ...
                    for _ in range(removed):
                        if len(self.snake) > 2:
                            self.snake.pop()
                    print("  💥 Snake mass reduced!")
                    time.sleep(0.1)
                break
    
    def get_keyboard_input(self):
        """Get keyboard input with Windows API"""
        try:
            import msvcrt
    ...
    def show_detailed_statistics(self):
        """Show detailed statistics with colors"""
        self.clear_screen()
        ...
        # Capital Analysis with colors
        win_rate = (self.win_count / max(1, self.trade_count)) * 100
        current_pnl = self.money - self.initial_money
        
        print(f"💰 Capital Analysis:")
        print(f"   Initial Capital: ${self.initial_money:,}")
        self.set_color(self.colors['GREEN'] if current_pnl >= 0 else self.colors['RED'])
    ...
    def run(self):
        """Main game loop with Windows native graphics"""
        self.set_color(self.colors['YELLOW'])
