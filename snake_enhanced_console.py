# Trading Snake - Enhanced Console Version
# No pygame required - uses Windows console API for better graphics
...
class EnhancedConsoleSnake:
    ...
    def clear_screen(self):
        """Clear screen with Windows API for better performance"""
        if os.name == 'nt':
            os.system('cls')
        else:
            os.system('clear')
    ...
    def set_cursor_position(self, x, y):
        """Set cursor position for Windows"""
        if os.name == 'nt':
    ...
    def draw_enhanced_interface(self):
        ...
        stats1 = f"💰 Capital: ${self.money:>10,} {pnl_color} P&L: {current_pnl:+9,}"
        ...
        if len(self.money_history) > 1:
            ...
            for i, money in enumerate(recent):
        ...
        # Controls
        controls = f"🎮 CONTROLS: ↑↓←→ or WASD to move | ESC to quit | SPACE to pause"
        print(f"  {controls}")
        
        # Direction indicator
        dir_display = {(1,0):"→", (-1,0):"←", (0,-1):"↑", (0,1):"↓"}
        current_dir = dir_display.get(self.dir, "?")
        print(f"  🧭 Current Direction: {current_dir} | 🖱️ Input: Arrow Keys/WASD")
        
        # Recent trades
        if self.trades and self.frame_count % 20 == 0:
            recent_trades = f"📜 Recent: "
        ...
        if len(self.food) < 4:
            while True:
                ...
                if (x,y) not in self.snake and not any((x,y)==(f[0],f[1]) for f in self.food):
                    ...
                    weights = [0.35, 0.35, 0.15, 0.1, 0.05]  # More profit opportunities
    ...
    def move_snake(self):
        ...
        # Wall collision - teleport with animation
        ...
        # Eat food with enhanced effects
        ate = False
    ...
    def take_trade_enhanced(self, trade_type):
        """Enhanced trade handling with better feedback"""
        values = {
            "profit": 1200, 
            "loss": -900, 
            "breakout": 1800, 
        ...
        pnl = int(base_pnl * random.uniform(0.8, 1.3))  # More variation
        ...
        if pnl > 0:
            ...
            # Snake growth
            if len(self.snake) < 10:  # Max length
    ...
    def teleport_effect(self):
        """Enhanced teleport with better feedback"""
    ...
    def show_statistics(self):
        ...
        print(f"   Winning: {self.win_count}")
        print(f"   Losing: {self.loss_count}")
